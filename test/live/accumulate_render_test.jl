...
#   • accumulate=true: six per-frame CAMERA moves fire ZERO RT2 resets
#     (non-structural), yet the rendered image still changes (the live diff
#     writes are applied) — the lit centroid MOVES;
#   • the first-frame pre-roll lands a converged (non-black) frame 1;
#   • a mid-sequence structural change (insert! a new plot) fires EXACTLY
#     ONE reset;
#   • default mode (accumulate=false) resets on EVERY one of the six camera
#     moves (the control that proves the gate changed nothing when the flag
#     is off).
...
# Off-centre red box + one directional light: a camera orbit visibly moves
# its screen position.
...
# Wrapped in a function (not a top-level loop) so the accumulator isn't a
# soft-scope local.
...
img1   = Makie.colorbuffer(screen)          # author reset + pre-roll folded in
c1     = lit_centroid(img1)
resets[] = 0                                 # ignore author reset; count motion
...
@testset "accumulate-across-frames: reset suppression + structural + preroll (subprocess)" begin
    ...
    # First frame converged (pre-roll landed): non-black (>300 lit px).
    ...
    # ...yet the image still changed — the per-frame diff writes were
    # applied (centroid moved).
    ...
    # Control: default mode resets on every one of the six moves (gate is a
    # no-op when off).
