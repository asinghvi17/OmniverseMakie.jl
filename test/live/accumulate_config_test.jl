...
#
# The behavioural render test (reset suppression + structural reset +
# preroll) is the GPU subprocess in accumulate_render_test.jl; this pins the
# config surface only.
...
@testset "accumulate-across-frames: ScreenConfig plumbing (pure)" begin
    # Field order matters: Makie.merge_screen_config constructs ScreenConfig
    # POSITIONALLY, so new fields must be trailing (selection_outline stays
    # 5th; `sensors` is 9th/last).
    ...
    # Positional constructor over all 9 fields (merge_screen_config builds
    # positionally).
    ...
    # Theme defaults resolve WITHOUT an override (off + preroll 40).
    ...
    # Documented surface exists; the reset observer is a zero-overhead no-op
    # by default.
