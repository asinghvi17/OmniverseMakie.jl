...
# ---------------------------------------------------------------------------
# M1.5 — integration capstone: a real Makie Scene with a mesh renders through
# ovrtx via Makie.colorbuffer / save.
#
...
#     assert eltype == RGBA{N0f8}, size ≥ 300², non-black > 1000, red-dominant
#   save(png, fig)                      # backend_show → colorbuffer → valid PNG
#
# Notes:
...
#   - save(fig) passes the figure ROOT scene (2-D PixelCamera); setup_scene! finds
#     the descendant Camera3D (the LScene) for the camera pose.
...
# --- save(fig, png): backend_show → our colorbuffer → valid PNG ---
save(png, fig; warmup = 48)
@assert isfile(png) "save(fig) wrote no file"
bytes = read(png)
println("PNG_BYTES=", length(bytes))
@assert length(bytes) > 1000 "PNG suspiciously small: \$(length(bytes)) bytes"
# PNG magic: 89 50 4E 47 0D 0A 1A 0A
@assert bytes[1:8] == UInt8[0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A] "not a PNG: \$(bytes[1:8])"

println("OK_MESH")
"""

@testset "M1.5 Mesh → colorbuffer / save (subprocess)" begin
    exitcode, output = run_ovrtx_subprocess(_M15_MESH_PROG; timeout = 900)
    @info "M1.5 subprocess output" output
    @test exitcode == 0
    ...
    # valid PNG written
    @test contains(output, "PNG_BYTES=")
end
