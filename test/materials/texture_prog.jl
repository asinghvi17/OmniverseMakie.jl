# Subprocess body for the image-texture INTEGRATION render (read + run by
# materials/texture_test.jl via run_ovrtx_subprocess).  Standalone .jl so the
# scene setup needs no escaping.
#
# Proves an image `color` is SAMPLED as a texture across the surface, not
# collapsed to a flat average.  A quad `mesh!` is textured with a 2-colour
# (red/blue) checker through the FULL Screen/colorbuffer pipeline:
#   - `mesh!(…; color = checker_img)` is MATERIALIZED →
#     `author_root_from_scene!` PRE-AUTHORS an OmniPBR material with
#     `asset inputs:diffuse_texture = @<temp PNG>@` (+ `project_uvw = 0`)
#     into /World/Looks, and the Mesh build branch authors the
#     `texCoord2f[] primvars:st` UV primvar from `:texturecoordinates` +
#     binds the material → the quad shows the checker.
# The render must contain BOTH a clearly RED-dominant region AND a clearly
# BLUE-dominant region.  A failed texture load (flat / black / averaged
# purple) shows NEITHER → the assertion fails.
...
# A 2-colour red/blue checker — a 2×2 LOGICAL checker SCALED to solid blocks
# so each colour owns a large pure region robust to bilinear texture
# filtering.
...
# A flat 3-D quad (z = 0) carrying per-vertex UVs + normals → Makie surfaces
# the UVs as `:texturecoordinates` (the `st` source).  3-D positions so
# `usda_mesh` authors point3f.
...
# Render the textured quad through the real pipeline.  A fresh Figure/LScene
# → its own Screen + stage; look straight down the quad normal (+z) so it
# fills the frame face-on.
fig = Figure()
...
# Classify each lit pixel as red-dominant / blue-dominant (a flat average
# is NEITHER).
...
# The pre-authored Looks scope must carry the diffuse_texture asset
# (open-time resolve).
