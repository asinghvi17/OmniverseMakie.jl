...
#
# A live `plot.color[]` / `plot.material[]` PARAM edit on a MATERIALIZED mesh
# re-writes the pre-authored OmniPBR shader inputs
# (`/World/Looks/Mat_<id>/Shader`) on the OPEN stage — the live diff path —
# WITHOUT re-authoring the root (`ROOT_OPENS == 1`):
#
#   color[]=:blue → exactly one :scaled_color push → exactly one
#     `diffuse_color_constant` shader write → BLUE-dominant
#   material[]=Attributes(metallic=0, roughness=0.9) → exactly one :material
#     push → metallic_constant + reflection_roughness_constant shader writes
#     → glossy specular collapses to MATTE (contrast drops)
#
# A true material SWAP to a brand-new material is NOT runtime-feasible (needs
# a root re-author), so "live material" means LIVE EDITS of the pre-authored
# material — NOT a `material:binding` re-bind.
#
# Phase 2 of the same prog repeats the material-param edit on a materialized
# *MeshScatter* (MS_-prefixed markers, per-phase ROOT_OPENS delta).
# Body: `material_live_prog.jl`.
# ---------------------------------------------------------------------------
...
@testset "live material edits: mesh + meshscatter (subprocess)" begin
    ...
    # Exactly the changed shader inputs were written (one write per changed
    # param).
    ...
    # Visible change: red build → blue after the color edit (live base
    # re-write).
    ...
    # glossy metal → matte dielectric: a substantial pixel change AND the
    # matte surface shows its pure diffuse base (metallic→0 removes the
    # white specular wash, so the blue base is markedly MORE saturated — a
    # robust, scene-independent signature of the edit).
    ...
    # The stage was authored EXACTLY ONCE across all three renders (live
    # writes only).
    mo = match(r"(?<!MS_)ROOT_OPENS=(\d+)", output)
    @test mo !== nothing && parse(Int, mo.captures[1]) == 1

    # --- Phase 2: material-param edit on a materialized MeshScatter. ---
    ...
    # The render CHANGED (glossy metal → matte dielectric) and the matte
    # surface shows its purer diffuse base (blue base MORE saturated once
    # the specular wash is gone).
    ...
    # Phase 2's stage authored exactly once too (per-phase delta — 2nd
    # Screen, same process).
