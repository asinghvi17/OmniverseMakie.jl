...
# ---------------------------------------------------------------------------
# M2.3 — USD subscene grouping (`def Scope` hierarchy mirroring the Makie tree).
#
...
# `/World/Scene_<id>/plot_<id>` instead of flat `/World/plot_<id>`.  Render stays
# pixel-equivalent (organizational grouping; plots keep their composed `:model_f32c`).
#
...
#       non-black with ROOT_OPENS==1, the LScene scene maps to a NESTED scope, and
#       the mesh's USD reference path is nested under that scope.
# ---------------------------------------------------------------------------
...
@testset "M2.3 scene_scopes_usda nested scope hierarchy (pure)" begin
    ...
    # --- scene2scope: root → /World (no extra Scene_ scope); subscenes nested ---
    ...
    @test !occursin("Scene_$(objectid(root))", usda)   # root is /World, no Scene_ scope
    ...
    @test occursin("    def Scope \"Scene_$(objectid(c2))\"", usda)
    @test !occursin("        def Scope \"Scene_$(objectid(c2))\"", usda)
    @test occursin("        def Scope \"Scene_$(objectid(gc))\"", usda)   # depth 2 (inside c1)
    ...
    # --- scope layers must OMIT upAxis (only the root governs — M1.2) ---
...
#
# RED (M2.2 flat layout): scene2scope maps every scene to "/World", so the
#   LScene scene's scope is "/World" (NOT "/World/Scene_<id>") and the mesh prim
#   path is "/World/plot_<id>" → the nested-path asserts fail.
# GREEN (M2.3): the figure root → /World, the LScene 3-D child → a Scene_ scope,
#   the mesh reference nested under it; render unchanged (non-black, opens==1).
# ===========================================================================
...
@testset "M2.3 nested-subscene render + paths (subprocess)" begin
    exitcode, output = run_ovrtx_subprocess(_M23_SUBSCENE_PROG; timeout = 900)
