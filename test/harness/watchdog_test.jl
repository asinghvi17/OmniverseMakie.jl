...
# PURE unit tests of the run_ovrtx_subprocess watchdog (helpers.jl): they
# spawn cheap single-process `julia` children (exit / sleep / crash) — NO
# ovrtx, NO GPU, no GPU lock needed — and assert the exit code is TRUTHFUL:
#   * 0    only on a real clean exit,
#   * -124 when the WATCHDOG times out and kills the child (negated
#          GNU-timeout convention — distinct from -9, which is reserved for
#          an EXTERNAL SIGKILL such as the OOM killer, so triage can tell
#          "it hung" from "something killed it"),
...
@testset "review E1: truthful watchdog" begin
    ...
    @testset "timeout => -124, stdout captured" begin
        # A plain sleeper that outlives the 2 s timeout; SIGTERM reaps it
        # within the grace.
        prog = "redirect_stderr(devnull); println(\"STARTED\"); flush(stdout); sleep(60)"
        exitcode, output = run_ovrtx_subprocess(prog; timeout = 2)
        @test exitcode == -124
        @test contains(output, "STARTED")   # output collected AFTER the reap
    end

    @testset "SIGTERM-ignoring child escalates to SIGKILL" begin
        # Julia routes SIGTERM to a dedicated sigwait thread, so libc
        # signal() alone can't trap it.  Unblocking SIGTERM on this thread +
        # SIG_IGN makes the kernel deliver-and-discard it here, so this
        # single process ignores SIGTERM — the watchdog must SIGKILL.
        prog = raw"""
        # sigset_t bit for SIGTERM(15)
        mask = zeros(UInt64, 16); mask[1] = UInt64(1) << (15 - 1)
        # SIG_UNBLOCK
        ccall(:pthread_sigmask, Cint, (Cint, Ptr{UInt64}, Ptr{Cvoid}), 1, mask, C_NULL)
        # SIG_IGN
        ccall(:signal, Ptr{Cvoid}, (Cint, Ptr{Cvoid}), 15, Ptr{Cvoid}(1))
    ...
    @testset "uncaught signal (crash) => -signal, not 0" begin
        # A child that dies from SIGSEGV on its own — the ovrtx-crash shape.
