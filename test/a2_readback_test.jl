...
#    no-y-flip, R,G,B,A channel order (dlpack.jl header, m1_orientation_test).
...
@testset "A2 with_mapped_hdr passthrough + unmap-on-throw (subprocess)" begin
    ...
    # (mirrors volumes_plot_test.jl): loop until the child reaches its first map.
    out = ""
    for _ in 1:4
        _, out = run_ovrtx_subprocess(_A2_HDR_PROG; timeout = 400)
        contains(out, "PASSTHROUGH_OK=") && break
    end
