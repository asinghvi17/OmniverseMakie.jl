...
# user-visible correctness the fix guarantees.  Built on the m6b_pick_test.jl pick shape.
# ---------------------------------------------------------------------------
...
@testset "A3 PathResolver cache invalidation on composition change (subprocess)" begin
    ...
    # (mirrors a2_readback_test.jl / m6b): loop until the child reaches its first pick.
    out = ""
    for _ in 1:4
        _, out = run_ovrtx_subprocess(_A3_RESOLVER_PROG; timeout = 500)
        contains(out, "PICK_A_IS_A=") && break
    end
