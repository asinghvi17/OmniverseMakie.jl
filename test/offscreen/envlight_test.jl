# Environment-light image (IBL) + background source + OmniPBR UV tiling:
# pure config/usda/texture-source/tiling-key checks, then GPU pixel proof
# (ovrtx silently ignores unknown attrs — pixels are the only oracle).
# :sky tripwire: standalone ovrtx does not implement the procedural sky, so
# the background stays black; if a future ovrtx renders it, remove the :sky
# warn in settings.jl and update this expectation.
...
@testset "envlight pure: config / dome usda / texture sources / tiling keys" begin
    @testset "ScreenConfig background plumbing" begin
        # positional ctor (sensors is last)
        @test :background in fieldnames(OM.ScreenConfig)
        ...
        @test !occursin("background", OM.rtx_settings_usda(mk(:default)))
    ...
    @testset "_env_dome_usda" begin
        ...
        # self-contained reference layer
        @test occursin("defaultPrim = \"EnvLight\"", d)
        # Makie 1.0 → USD 1000 scale
        @test occursin("float inputs:intensity = 1500.0", d)
    ...
    @testset "_env_texture_file: matrix → fresh temp PNG; path passthrough" begin
        ...
        # HDR components clamp (N0f8 conversion of >1 would throw) — warns,
        # does not throw
    ...
    @testset "EnvironmentLight no longer root-baked" begin
        # The dome is authored via a removable reference (envlight.jl) so it
        # can be live-swapped; usda_light emits nothing for EnvironmentLight.
    ...
    @testset "OmniPBR UV tiling keys + float2 emitter" begin
        ...
        # NTuple{2} → float2
        @test occursin("float2 inputs:texture_scale = (4.0, 4.0)", usda)
        # Vec2f normalized
        @test occursin("float2 inputs:texture_translate = (0.1, 0.2)", usda)
        @test occursin("float inputs:texture_rotate = 45.0", usda)

        # color3f inputs are Float32-converted like the float2/float branches:
        # a raw Float64 3-tuple must emit Float32 digits, not full Float64.
        c3 = OM.usda_omnipbr_material("M3", Dict{String,Any}("emissive_color" => (1 / 3, 2 / 3, 0.1)))
        @test occursin("color3f inputs:emissive_color = (0.33333334, 0.6666667, 0.1)", c3)
        @test !occursin("0.3333333333333333", c3)   # no leaked Float64 digits
...
corner(img)   = img[8, 8]   # top-left = background region (sphere is centered)
...
# 4) :sky TRIPWIRE — standalone ovrtx does not render the procedural sky
#    (background stays black).  Camera looks HORIZONTALLY so the frame top is
#    above the horizon (where a sky would be).
...
push_environment_image!(scr5, green_env)          # pre-colorbuffer → stashed
img5 = Makie.colorbuffer(scr5)
println("STASH_GREEN=", greendom(img5))
close(scr5)

# 6) UV tiling is honored: project_uvw + texture_scale changes a textured
#    plane's rendering
...
@testset "envlight GPU: illuminate / live swap / backgrounds / stash / tiling (subprocess)" begin
    ...
    # env light illuminates
    @test getint("ENV_GREEN")  !== nothing && getint("ENV_GREEN")  > 5000
    # live swap took ...and green is gone
    @test getint("SWAP_RED")   !== nothing && getint("SWAP_RED")   > 5000
    @test getint("SWAP_GREEN") !== nothing && getint("SWAP_GREEN") < 100
    @test contains(out, "TMP_CLEANED=true")   # bounded temps
    @test contains(out, "DOME_BG_CORNER_GREEN=true")   # :domelight background
    # :sky tripwire (see header)
    @test getint("SKY_TOP_LIT") !== nothing && getint("SKY_TOP_LIT") == 0
    # pre-display push applied
    @test getint("STASH_GREEN") !== nothing && getint("STASH_GREEN") > 5000
    # textured plane rendered
    @test getint("TILING_NB")   !== nothing && getint("TILING_NB")   > 5000
    # tiling inputs honored
    @test getint("TILING_DIFF") !== nothing && getint("TILING_DIFF") > 1000
end
