...
# ---------------------------------------------------------------------------
# PathResolver lifetime: the cached `screen.path_resolver` captures the ovrtx
# path dictionary as of the current stage composition, so every composition
# change (add_usd_reference! / remove_usd!: plot delete/insert, empty!,
# volume reload) must drop it — a stale cache can resolve a pick after
# delete+re-add against the old dictionary. Subprocess; GPU, no IndeX/GLMakie.
# ---------------------------------------------------------------------------
...
# --- composition change 2: ADD a NEW mesh B at the same spot ---
# (register → add_usd_reference!)
...
# --- composition change 3: add C while the resolver is cached (insert seam)
...
@testset "PathResolver cache invalidation on composition change (subprocess)" begin
    ...
    @test contains(out, "PICK_A_IS_A=true")    # baseline pick resolves plot A
    @test contains(out, "RESOLVER_CACHED_AFTER_PICK_A=true")
    # The deterministic invalidation assertions:
    @test contains(out, "RESOLVER_INVALIDATED_ON_DELETE=true")
    @test contains(out, "RESOLVER_INVALIDATED_ON_INSERT=true")
    # The user-visible correctness: a pick after delete+re-add resolves the
    # NEW plot.
