...
# attach_picking! interaction (subprocess, GLMakie). A selection-outline
# Screen is LdrColor-only while the live viewport presents HdrColor, so
# interactive_display(selection_outline=true) refuses; the data flow (click
# → pick_hit → selected[] + on_hit) works fully. Picks run synchronously via
# _pick_at! (render_tick detached) — synthetic GLFW mouse events are racy.
...
# 1. interactive_display(selection_outline=true) must REFUSE with an
#    actionable error (the deferred LdrColor live-present), BEFORE building
#    any viewport.
threw = false; msg = ""
try
...
# 2. Default viewport (outline off) — a normal HdrColor Screen.
session = OM.interactive_display(fig; size = (200,200))
# Detach the live render_tick so background auto-ticks cannot step the
# renderer while we pick on the main task.
off(session.tick_listener)

# 3. Attach with outline=true → it DEGRADES (the viewport Screen has no
#    selection_outline), warns once, and falls back to outline=false; the
#    data flow stays fully live.  on_hit records hits.
on_hits = Any[]
...
# 4. Drive the pick handler SYNCHRONOUSLY at the centre (bypassing the GLFW
#    event thread).
...
# 5. A far corner → background → selected[] === nothing (and on_hit still
#    fires, with nothing).
...
# 7. A throwing on_hit inside a REAL mousebutton event must not escape the
#    click listener: notify(::Observable) does not catch listener errors, so an
#    uncaught throw would kill GLMakie's render task.  Fire press+release (no
#    drag) through the mousebutton Observable; the guard must contain it.
boom = OM.attach_picking!(session; on_hit = _ -> error("boom in on_hit"))
session.glscene.events.mouseposition[] = (100.0, 100.0)
listener_threw = false
try
    ...
    global listener_threw = true
end
println("THROWING_PICK_CONTAINED=", !listener_threw)
OM.detach_picking!(boom)
...
@testset "attach_picking! (subprocess, GLMakie)" begin
    ...
    @test exitcode == 0
    # interactive_display(selection_outline=true) refuses with the LdrColor
    # follow-up message.
    ...
    # A throwing pick handler is contained (guarded listener; render task lives).
    @test contains(output, "THROWING_PICK_CONTAINED=true")
