...
println("MAP_CUDA_OK=", mapping.data != C_NULL, " W=", mapping.width, " H=", mapping.height, " C=", mapping.channels)
println("WAIT_EVENT_NONZERO=", mapping.wait_event != Csize_t(0))
OV.unmap_cuda(mapping)
println("MAPPING_CLOSED=", !mapping.open)
...
@testset "OV.map_cuda (subprocess, CUDA linear)" begin
    ...
    @test contains(output, "MAPPING_CLOSED=true")
