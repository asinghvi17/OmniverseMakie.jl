...
# M6.A Task 4 — GPU-direct blit: interactive_display(...; gpu_direct=true) shows a non-black
# RTX frame through the on-device CUDA→GL path (no CPU roundtrip).
#
# CUDA↔GL interop for a resource must run on ONE task/context (spike §2): the GLMakie
# render task, where the auto `render_tick` listener fires `present!` with the GL context
# current.  So the frame is driven via `colorbuffer` (which runs the render loop / fires
# the tick on the render task) — NOT a manual `on_render_tick!` from the main task (that
# would register/map in a different CUDA context → segfault).
#
# To guarantee the GPU path GENUINELY ran (no silent CPU fallback), assert (a) the frame is
# non-black AND (b) the session is still on the :gpu blitter with a registered gpu_state —
# a CPU fallback flips session.blitter to :cpu.
...
# Drive the GPU present via the render loop (render task, GL current); two frames so the
# lazy register + a steady GPU blit have both run.
...
@testset "M6 GPU-direct blit (subprocess, CUDA+GL)" begin
    exitcode, output = run_ovrtx_subprocess(_M6_GPUBLIT_PROG; timeout = 600, retries = 2, ready_marker = "OK_GPU_BLIT")
    @info "M6 gpu blit output" output
    @test exitcode == 0
    @test contains(output, "CUDA_FUNCTIONAL=true")
    @test contains(output, "OK_GPU_BLIT")
    @test contains(output, "GPU_BLITTER=gpu")        # genuinely used the GPU path (no fallback)
