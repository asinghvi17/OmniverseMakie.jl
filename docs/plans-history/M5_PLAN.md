# Milestone M5 — Interactive RTX viewport (bite-sized plan)
...
- **M2 dependencies:** the per-frame camera write uses M2.4's persistent `map_attribute` binding when available (zero-copy fixed-size xform) — M5 falls back to the M0 `write_attribute` path if M2.4 isn't merged. M2.6's benchmark sets the interactivity bar (if CPU-side writes miss interactive rates, M5.4 GPU-direct becomes mandatory, not optional).

## File structure (M5 adds)

```
src/
  interactive/
    viewport.jl     # NEW: interactive_display(fig/scene)->ViewportSession; the GLMakie host window + image! + render_tick hook
    blit.jl         # NEW: cpu_blit!(tex, frame) (v1); cuda_gl_blit!(session, …) (v2, M5.4)
    camera_loop.jl  # NEW: on_render_tick!(session) — read Camera3D → sync_camera! → reset-on-change → step (bounded) → blit; accumulation state
  binding/OV.jl     # MODIFY: bounded step!(r; timeout_ns); CUDA-array map mode (OVRTX_MAP_DEVICE_TYPE_CUDA_ARRAY) for M5.4
  screen.jl         # MODIFY: ViewportSession owns the open-stage Screen + the GLMakie screen + the render_tick listener (teardown order)
test/
  m5_camera_loop_test.jl   m5_blit_test.jl   m5_viewport_test.jl   m5_interop_test.jl
bench/
  interactive.jl    # NEW (M5.3/M5.4): sustained interactive FPS under an orbit, CPU vs GPU-direct
```

---

## Task M5.1 — Viewport window + static ovrtx frame on screen (CPU blit) ★

**Files:** `src/interactive/viewport.jl`, `src/interactive/blit.jl`. Test: `test/m5_blit_test.jl` (offscreen, no window), `test/m5_viewport_test.jl` (real-window integration).
...
- [ ] **Step 1 (failing offscreen test, blit):** `test/m5_blit_test.jl` — make a `400×400` `RGBA{N0f8}` frame (red top half, blue bottom), an offscreen GLMakie `Screen(visible=false)` with `image!`, call `cpu_blit!`, read the texture back (`GLMakie.GLAbstraction.gpu_data(tex)` or `colorbuffer(glscreen)`), assert the top is red / bottom blue (and orientation matches our top-left-origin, no flip). RED (`cpu_blit!` undefined).
- [ ] **Step 2 (window integration):** `test/m5_viewport_test.jl` (subprocess) — `interactive_display` of a 1-mesh `LScene`; assert the window opened (`glscreen` is open) and `colorbuffer(glscreen)` is non-black (the blitted RTX frame is visible). The user can watch this window live. RED.
...
- [ ] **Step 6:** commit `feat(M5.1): interactive viewport window + CPU blit of the ovrtx frame`.

**Acceptance:** a real window shows the static RTX render of a Makie scene; `cpu_blit!` puts a host frame on screen right-side-up.

---

## Task M5.2 — Live camera interaction loop (orbit/pan/zoom → ovrtx) ★

**Files:** `src/interactive/camera_loop.jl`, `src/binding/OV.jl` (bounded `step!`). Test: `test/m5_camera_loop_test.jl` (offscreen/component), `m5_viewport_test.jl` (synthetic-event integration).
...
- [ ] **Step 6:** commit `feat(M5.2): live camera interaction loop (orbit→write_xform!→step→blit) + bounded step`.
...
## Task M5.3 — Progressive RT2 accumulation (refine while idle, reset on interaction)

**Files:** `src/interactive/camera_loop.jl`. Test: `test/m5_camera_loop_test.jl` (extend, component).
...
- [ ] **Step 4:** run → PASS.
- [ ] **Step 5:** commit `feat(M5.3): progressive RT2 accumulation — idle-refine, reset-on-interaction`.

**Acceptance:** a still view visibly refines over frames; any interaction restarts accumulation.

---

## Task M5.4 — GPU-direct CUDA-GL interop blit (zero host roundtrip) ★

**Files:** `src/interactive/blit.jl`, `src/binding/OV.jl` (CUDA-array map mode). Test: `test/m5_interop_test.jl` (windowed — needs a real GL context + CUDA).
...
- ⚠️ **Validate-first (Step 1):** the whole interop in the *live loop* is the note's flagged risk ("integration glue"). Step 1 proves a single map→copy→unmap puts the right pixels in the texture (compare against the CPU-blit frame). If it doesn't (context/sync/format), STAY on the CPU blit (fully working from M5.1) and record the gap for a follow-up — do NOT block the milestone on GPU-direct.
- [ ] **Step 1 (failing interop test, windowed):** `test/m5_interop_test.jl` — render one frame; `cuda_gl_blit!` it; read the texture back and assert it matches the CPU-blit of the same frame within tolerance (correct pixels via the on-device path). RED.
- [ ] **Step 2:** run → FAIL.
...
- [ ] **Step 5:** commit `feat(M5.4): GPU-direct CUDA-GL interop blit (CUDA-array → GL texture)`.
...
## Task M5.5 — Resize, teardown, robustness

**Files:** `src/interactive/viewport.jl`, `src/screen.jl`. Test: `test/m5_viewport_test.jl` (extend).
...
- [ ] **Step 4:** run → PASS.
- [ ] **Step 5:** commit `feat(M5.5): viewport resize + leak-free teardown + render-loop error guard`.
...
## Task M5.6 — Interactive throughput benchmark (gate)
...
- [ ] **Step 4:** commit `bench(M5.6): interactive throughput (FPS + latency, CPU vs GPU-direct)`.

**Acceptance:** the interactive loop sustains an interactive frame rate on the A5000 (or the shortfall is measured + escalated).

---

**M5 GATE:** a Makie scene opens in a real interactive window showing the live ovrtx path-traced render; mouse orbit/pan/zoom reframes it live via per-frame `write_xform!` on the open stage; the image progressively refines when idle and resets on interaction; frames reach the screen via CPU blit (guaranteed) and GPU-direct CUDA-GL interop (when it passes); resize + teardown are clean; throughput meets interactive rates. ✅ → M6 (GPU-direct CUDA-GL blit + AOV picking + subscene hardening).
...
2. **CUDA-GL interop works inside the live render-task loop** (M5.4 Step 1 — the note's flagged "integration glue" risk). Fallback: ship the CPU blit (fully working from M5.1); GPU-direct is a follow-up.
3. **ovrtx exposes the `CUDA_ARRAY` map mode over the C ABI from Julia** (M5.4) — the note says yes (C ABI == whole API; `OVRTX_MAP_DEVICE_TYPE_CUDA_ARRAY`, Python can't but ccall can). Fallback: CUDA-linear (`Device.CUDA`) device→array copy, or the CPU blit.
4. **A bounded `enqueue_wait` exists / can be added** without destabilizing the step (M5.2). Fallback: poll `ovrtx_get_status` with a deadline.
5. **GLMakie's `Camera3D` mouse interaction + `render_tick` hook compose with our blit** (M5.2) — i.e. driving the camera and injecting per-frame work via `render_tick` (no `Consume`) behaves. Fallback: a custom GLMakie `renderloop` (note §2) or feed GLFW events to `Camera3D` manually.
6. **(M2 dependency)** M2.4's persistent `map_attribute` binding for the camera xform lands — M5 uses it for the per-frame write; until then M5 uses the M0 `write_attribute` path (correct, non-zero-copy).
