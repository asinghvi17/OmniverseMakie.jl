# Ported from references/RPRMakieNotes/scripts/transparentMaterial.jl
# (Lazaro Alonso). Earth-textured transparent sphere (opacity=0.75) over a
# white inner sphere + gainsboro plane. UberMaterial(transparency=Vec4f(0.25))
# → material=(; opacity=0.75f0); earth texture via color=earth_img.
...
function scene_transparentMaterial()
    ...
    # PointLight: color first, then position (OmniverseMakie convention)
    ...
    # Outer earth sphere — UberMaterial(transparency=Vec4f(0.25)) →
    # opacity=0.75; texture via color=img
