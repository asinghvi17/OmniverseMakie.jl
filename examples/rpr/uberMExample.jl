...
# Earth-textured sphere (UberMaterial metallic=0/roughness=0.1) on a white
# plane under a grey dome. RPR.UberMaterial → material=(; metallic,
# roughness); color=earth_img auto-emits diffuse_texture. PointLight arg
# order swapped: original was position-first; OmniverseMakie wants
# color-first.
...
function scene_uberMExample()
    ...
    # Earth sphere: UberMaterial(reflection_metalness=0.0,
    # reflection_roughness=0.1) → material=(; metallic=0.0f0,
    # roughness=0.1f0); color=earth_img → auto diffuse_texture
