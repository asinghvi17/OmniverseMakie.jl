# Ported from references/RPRMakieNotes/scripts/earth_ina_julia_box.jl
# (Lazaro Alonso). Earth-textured sphere inside a coloured Cornell box with
# 4 emissive corner lights. EmissiveMaterial corners →
# material=(; emissive=c); earth texture via circshift+color=img.
...
function scene_earth_ina_julia_box()
    ...
    # Earth sphere — texture via circshift + color = img (backend
    # auto-emits diffuse_texture + st)
