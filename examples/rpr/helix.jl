...
# DNA double-helix: two strands of coloured spheres (A/T/C/G base-pairs), connecting
# two-tone rods, letter-glyph legend meshes (Luxor), and an image-textured portrait quad.
# PointLight arg order swapped: original was position-first; OmniverseMakie wants color-first.
# GLMakie.rotate!/RPRMakie.rotate! → Makie.rotate! (re-exported via OmniverseMakie).
# DiffuseMaterial → drop material=; UberMaterial(metalness, roughness) → material=(; metallic, roughness).
...
function scene_helix()
    ...
    colors2 = [i == 1 ? 2 : i == 2 ? 1 : i == 3 ? 4 : 3 for i in colors1]  # complementary pairs
    ...
    planeL = Rect3f(Vec3f(-10,  -2, 17.5),  Vec3f(20, 30, 0.05))   # emissive back panel
    ...
    # ── Portrait texture quad (DiffuseMaterial → drop material=, color=img) ────────
