...
- **Updated after the original M0 plan:** no Python/PythonCall and no wheel discovery remain true. The native lib is still found by `Libdl.dlopen`, but resolution is now `ENV["OVRTX_LIBRARY_PATH"]` → in-repo `OVRTX_jll` artifact wrapper over NVIDIA's official C-library release archive → default soname.
