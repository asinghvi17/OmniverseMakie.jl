...
#
# The camera is baked once at author time, then updated live via
# sync_camera!'s write_xform! — no stage re-open on a pose change. (A FOV
# change is a `float focalLength` scalar write; orbit/pan/zoom is the xform.)
...
4x4 camera-to-world transform in USD row-vector convention: rows 1-3 are the
camera basis (x/right, y/up, z/back-toward-camera), row 4 is the eye position
(weight 1) — the layout `write_xform!` and USD `matrix4d` expect.

Right-handed: +Z = normalize(eye-target) (toward viewer),
...
Format a 4x4 row-vector matrix (basis in rows 1-3, translation in row 4) as
a USD `matrix4d` literal, rows emitted as-is (no transpose). Contrast
`usda_matrix4d`, which transposes a Makie column-vector matrix first.
"""
...
# ------------------------------------------------------------------
# _validate_camera_path — guard the camera prim path
# ------------------------------------------------------------------

"""
    _validate_camera_path(camera_path) -> camera_path

Return `camera_path` if it is a direct `/World/<name>` child
(e.g. `"/World/Camera"`); else throw. Enforces consistency with
`author_render_root!`, which creates the camera prim as a direct `/World`
child from the last path segment.
"""
function _validate_camera_path(camera_path::String)
    ...
    if length(parts) == 3 && parts[1] == "" && parts[2] == "World" && !isempty(parts[3])
        # Depth alone is not enough: "/World/My Camera" has the right shape
        # but "My Camera" (space) is not a legal USD identifier and would
        # author a broken prim. Each segment must validate as an identifier.
...
USD camera intrinsics from a vertical FOV (degrees) and image size (W x H
px). Returns `(focal_length, h_aperture, v_aperture)`, from the USD FOV
definition `vfov = 2*atan(v_aperture / (2*focal_length))`:
  focal_length = v_aperture / (2*tand(fov_deg/2))
  h_aperture   = v_aperture * (W/H)
`v_aperture = 15.2908` (fixed).
"""
...
Initial bake of `scene`'s 3-D camera pose + lights into the USD render-root,
delegating to `author_root_from_scene!` (usd.jl, reads the camera and
`scene.compute[:lights][]`). Later pose changes go through `sync_camera!`
(write_xform!), not a re-bake.

Re-opens the stage on every call: all previously added USD references are
lost and must be re-added, then call `OV.reset!(screen.renderer)` before
rendering. Precondition: `scene` has a 3-D camera controller. `camera_path`
...
# Snapshot of the scene's 3-D camera pose + FOV, or `nothing` if the scene
# has no 3-D camera controller (a 2-D / pixel camera has no
# eyeposition/lookat/up/fov).
...
Diff the scene's 3-D camera pose/FOV against `screen.last_camera`. On a pose
change push `write_xform!(camera_path, camera_to_world(...))`; on a FOV
change push a `focalLength` scalar write. Updates `last_camera`; returns
`true` iff anything was written (so `colorbuffer` knows to `OV.reset!`).
Returns `false` when the scene has no 3-D camera or nothing changed (a
